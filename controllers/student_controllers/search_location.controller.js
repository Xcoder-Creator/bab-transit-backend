...
const search_location = async (req, res) => {
    ...
    if (req.body){
        ...
        if (form_data.search){
            ...
            var search = sanitize_data(form_data.search);
            //-------------------------------------------------------------------
                        ...
                        if (err){
                            ...
                            if (result1.status === false){
                                ...
                                if (result1.data.length > 0 && result1.data.length === 1){
                                    let result2 = await db_query.search_dropoff_location(search);
                                    ...
                                    if (result2.status === false){
                                        ...
                                        if (result2.data.length > 0){
                                            let locations = result2.data;

                                            res.statusCode = 200;
                                            res.json({ err_msg: null, result: locations, succ_msg: `${locations.length} result${(locations.length > 1) ? 's' : ''} found for '${search}'`, res_code: 200 });
                                        } else {
                                            res.statusCode = 404;
                                            res.json({ err_msg: `No result found for '${search}'`, result: null, succ_msg: null, res_code: 315 });
                                        }
                                    }
                                } else {
                                    res.statusCode = 404;
