...
const verify_account = async (req, res) => {
    ...
    if (req.body){
        ...
        if (form_data.student_id && form_data.verification_code){
            ...
            if (student_id !== NaN){
                if (verification_code.length > 0){
                    let result1 = await db_query.check_student_verification_code(student_id, verification_code);
                    ...
                    if (result1.status === false){
                        ...
                        if (result1.data.length > 0 && result1.data.length === 1){
                            ...
                            jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err, user) => {
                                if (err){
                                    let result2 = await db_query.delete_student_verification_code(student_id, verification_code);
                                    ...
                                    let result2 = await db_query.verify_student_account(student_id);
                                    ...
                                    if (result2.status === false){
                                        ...
                                        let result3 = await db_query.delete_student_verification_code(student_id, verification_code);

                                        if (result3.status === false){
                                            res.statusCode = 400;
                                            res.json({ err_msg: 'An error just occured, Try again later', result: null, succ_msg: null, res_code: 400 }); 
                                        } else if (result3.status === true){
                                            let result4 = await db_query.get_all_student_details(student_id);
